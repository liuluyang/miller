...
# cipher.update(b'miller.json')   # 你可以理解为加了盐  salt
# 这个东西 可有可无，完全看需求来。
...
# data = "这就是我的加密的数据".encode("utf-8")  # 这个就是你要进行加密的数据了。 注意这是二进制的数据。
...
# 3
nonce = cipher.nonce
# 4 这个就是你要进行加密的数据了。 注意这是二进制的数据。
data = "这就是我的加密的数据".encode("utf-8")
# 5 对数据进行加密
ciphertext, tag = cipher.encrypt_and_digest(data)

# 6 对加密后的内容进行序列化存储, 使用base64
json_k = ["key", "nonce", "ciphertext", "tag"]
json_v = [b64encode(i).decode("utf-8") for i in [key, nonce, ciphertext, tag]]

import json
json_dic = json.dumps(dict(zip(json_k, json_v)))
print(json_dic)
